...
export function* addUserCoinSaga(action) {
  ...
  try {
    yield userCoinsRef(action.uid).push(action.coin);
...
export function* updateUserCoinSaga(action) {
  ...
  try {
    yield userCoinRef(action.uid, action.coinId).update({
      quantity: action.newQuantity,
      price: action.newPrice,
    });
    yield put(
      actions.updateUserCoinSuccess(
        action.coinId,
        action.newQuantity,
        action.newPrice
      )
...
export function* fetchUserCoinsSaga(action) {
  ...
  try {
    ...
    yield userCoinsRef(action.uid).once("value", (snapshot) => {
      const userCoinsObj = snapshot.val();
      if (userCoinsObj) {
        fetchedCoins = Object.keys(userCoinsObj).map((coinId) => ({
          ...userCoinsObj[coinId],
          coinId,
        }));
      }
