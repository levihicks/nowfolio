...
const CoinChart = props => {
    ...
    useEffect(() => {   
        ...
        setLoading(true);
        coinbaseAxios.get(`products/${currentCoin}/candles?granularity=${granularity}`)
            .then(res => {
    ...
    let content = <div style={{minHeight: "300px"}}><Spinner /></div>;
    if (data.length > 0 && !loading) {
        ...
        content = (
            <ResponsiveContainer width="100%" height={400}>
                <LineChart 
                    data={data}
                    margin={{top:20, bottom: 20, right: 5}}>
                    <XAxis dataKey="Name" />
                    <YAxis width={100} domain={[
                        Number(floor.toFixed(8).toString()), 
                        Number(ceiling.toFixed(8).toString())
                    ]}/>
                    <Tooltip />
